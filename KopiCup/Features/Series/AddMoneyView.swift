...
struct AddMoneyView: View {
    ...
    private var placeholderText: String {
        let code = viewModel.goal?.currency ?? viewModel.currencyCode
        let symbol = currencySymbol(for: code) ?? code
        ...
        HStack(spacing: 10) {
            ForEach(quickRow1, id: \.self) { amount in
                Button(action: { applyQuickAmount(amount) }) {
                    Text("+\(amount) \(currencySymbol(for: viewModel.goal?.currency ?? viewModel.currencyCode) ?? "₽")")
    ...
    private var quickSingleButton: some View {
        Button(action: { applyQuickAmount(quickSingle) }) {
            Text("+\(quickSingle) \(currencySymbol(for: viewModel.goal?.currency ?? viewModel.currencyCode) ?? "₽")")
                .font(.system(size: 15, weight: .semibold, design: .rounded))
                .foregroundColor(.primary)
                .frame(maxWidth: .infinity)
                .padding(.vertical, 12)
